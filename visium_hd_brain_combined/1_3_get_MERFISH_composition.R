...
merfish_cells_one_section <- merfish_cells %>% 
  filter(brain_section_label == "C57BL6J-638850.40") %>% 
  mutate(class = str_replace_all(class, "[- ]", ""))

ggplot(merfish_cells_one_section, aes(y=y_section, x=x_section, color=class)) +
...
region_annot_label <- region_annotations %>% 
  group_by(acronym_lvl5) %>% 
  summarise(median_x = median(pxl_col_in_lowres),
            median_y = median(pxl_row_in_lowres))

p_regions <- ggplot(region_annotations, aes(y=pxl_row_in_lowres, x=pxl_col_in_lowres, fill=acronym_lvl5)) +
  ...
  theme(legend.position = "none",
        axis.title = element_blank())
p_regions
ggsave("visium_hd_brain/plots/annotated_regions_lvl5.png", p_regions,
...
region_annotations$acronym_lvl6 %>% table
...
merfish_cells %>% filter(parcellation_division %in% unique(region_annotations$acronym_lvl5)) %>% nrow
 
merfish_cells_filtered <- merfish_cells %>% 
  filter(parcellation_division %in% unique(region_annotations$acronym_lvl5)) %>%
