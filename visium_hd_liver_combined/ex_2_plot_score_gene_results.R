...
for (dataset_name in c("sca002", "caw009")){
  ...
  for (st in score_type){
    ...
    p_props_list <- lapply(gf_roi, function(txt) {
      ...
      square_size <- scores_df %>% filter(gene_filter == gf, roi == roi_i) %>%
        distinct(square_size) %>% pull(square_size)
      score_df_subset <- scores_df %>% filter(gene_filter == gf, roi == roi_i, score==st) %>%
        mutate(proportion=1,
               celltype = factor(celltype, levels = celltype_order)) 
        
      p <- plot_scatterbar(score_df_subset,
