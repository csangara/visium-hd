...
scrublet_res <- read.csv("visium_hd_brain_ff/Visium_HD_MouseBrain_FF_008um/scrublet_Visium_HD_MouseBrain_FF_008um.csv",
                         header = TRUE) %>% rename(spot_id = X)

scrublet_res$predicted_doublet %>% .[. != ""] %>% table # Only four doublets...

# About 25% of bins have two nuclei (see later), so let's find the threshold for 75% of data
q75_scrublet <- scrublet_res$doublet_score %>% .[!is.na(.)] %>% quantile(probs = 0.75)

# Plot histogram of scrublet scores
ggplot(scrublet_res, aes(x=doublet_score)) +
  geom_histogram(bins=50) +
  geom_vline(xintercept=q75_scrublet, color="red", linetype="dashed")+
  theme_minimal()

scrublet_res <- scrublet_res %>% mutate(predicted_doublet = ifelse(doublet_score > q75_scrublet, "True", "False"))
...
ggplot(intersection, aes(x=nucleus_area_bin, y=total_counts)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
ggsave("visium_hd_brain_ff/plots/boxplot_counts_by_nucleas_area.png",
...
ggsave("visium_hd_brain_ff/plots/barplot_nucleus_count_per_bin.png",
       width=7, height=5, dpi=300, bg="white")

# Is there the same trend for more nuclei? -> in this case, trends are less clear
ggplot(intersection, aes(x=nucleus_area_bin, y=total_counts)) +
  geom_boxplot() +
  theme_minimal() +
...
p_nuclei_count <- SpatialFeaturePlot(visium_obj, features="nuclei_count", image.alpha=0, pt.size.factor = 2) +
  scale_fill_gradientn(colours=RColorBrewer::brewer.pal(9, "BuPu"),
                       breaks=seq(0, 10, by=2))
ggsave("visium_hd_brain_ff/plots/spatialfeatureplot_nuclei_count.png",
       rotate_image(p_nuclei_count, -90), width=7, height=7, dpi=300, bg="white")

p_nuclei_count_group <- SpatialDimPlot(visium_obj, group.by = "nuclei_count_group", image.alpha=0, pt.size.factor = 2) +
...
ggsave("visium_hd_brain_ff/plots/spatialdimplot_nuclei_count_group.png",
       rotate_image(p_nuclei_count_group, -90), width=7, height=7, dpi=300, bg="white")
...
ggsave("visium_hd_brain_ff/plots/barplot_rctd_spotclass_per_n_nucleus.png",
       width=7, height=5, dpi=300, bg="white")

intersection_join_scrub <- inner_join(nuclei_presence_df, scrublet_res %>% filter(!is.na(doublet_score)), by = c("spot_id" = "spot_id")) %>%
  # Only keep if nucleus_area_percentage > threshold
  filter(nucleus_area_percentage > nucleus_pct_threshold) #%>% 
  # Only keep grid_id where the remaining spots matches with nucleus_count
...
barplot_abs + barplot_rel +
  # gather legends
  plot_layout(guides = "collect")
ggsave("visium_hd_brain_ff/plots/barplot_scrublet_per_n_nucleus.png",
       width=7, height=5, dpi=300, bg="white")
...
# Get auc
print(paste0("AUROC: ", subset(auc(curves), curvetypes=="ROC")$aucs)) #0.505
print(paste0("AUPRC: ", subset(auc(curves), curvetypes=="PRC")$aucs)) #0.235
