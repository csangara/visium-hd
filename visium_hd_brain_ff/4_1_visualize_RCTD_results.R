...
deconv_props <- read.table("visium_hd_brain_ff/Visium_HD_MouseBrain_FF_008um/proportions_rctd_Visium_HD_MouseBrain_FF_008um",
                           header = TRUE)
...
ggsave("visium_hd_brain_ff/plots/spatialdimplot_celltype.png",
       rotate_image(p_celltype, -90),
       width = 8, height = 6, bg = "white")

# Get region annotations
region_annotations <- readRDS("data/Visium_HD_MouseBrain_FF/tissue_positions_with_annotations_008um.rds")
...
p_regions <- ggplot(region_annotations,
                    aes(y=pxl_row_in_lowres, x=pxl_col_in_lowres, fill=division)) +
  geom_bin2d(bins=500) +
...
p_regions
ggsave("visium_hd_brain_ff/plots/annotated_regions.png",
       rotate_image(p_regions, 90),
...
p
# ggsave(paste0("visium_hd_brain_ff/plots/barplot_avg_by_region", ext, ".pdf"), p,
#        width = 8, height = 6, bg = "white")
ggsave(paste0("visium_hd_brain_ff/plots/barplot_avg_by_region.png"), p,
       width = 8, height = 6, bg = "white")
...
#p_boxplot_all
ggsave(paste0("visium_hd_brain_ff/plots/boxplot_all.png"), p_boxplot_all,
       width = 15, height = 8, bg = "white")


# DOUBLET MODE
doublet_props <- deconv_props
...
p_spot_class <- SpatialDimPlot(visium_obj_subset, group.by = "spot_class",
  ...
  guides(fill = guide_legend(override.aes = list(size = 3))) +
  theme(legend.position = "right")
ggsave("visium_hd_brain_ff/plots/spatialdimplot_spot_class.png",
...
p_region_spotclass <- ggplot(doublet_info_region, aes(x = division,
  ...
  theme(axis.title = element_blank(),
        panel.grid.major.x = element_blank())
#p_region_spotclass
ggsave("visium_hd_brain_ff/plots/barplot_spot_class_by_region.png",
       p_region_spotclass,
       width = 8, height = 6, bg = "white")
