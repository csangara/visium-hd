...
#p_boxplot_all
ggsave(paste0("visium_hd_brain_ff/plots/boxplot_all", ext, ".png"), p_boxplot_all,
...
doublet_info <- read.table(paste0("visium_hd_brain_ff/Visium_HD_MouseBrain_FF_008um/proportions_rctd_Visium_HD_MouseBrain_FF_008um", ext, "_doublet_info.tsv"),
                           header = TRUE)
...
p_spot_class <- SpatialDimPlot(visium_obj_subset, group.by = "spot_class",
  ...
  guides(fill = guide_legend(override.aes = list(size = 3))) +
  theme(legend.position = "right")
ggsave(paste0("visium_hd_brain_ff/plots/spatialdimplot_spot_class", ext, ".png"), rotate_image(p_spot_class, -90),
       width = 8, height = 6, bg = "white")
